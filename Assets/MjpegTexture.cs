...
/// <summary>
/// An unused Unity3D Script to display Mjpeg streams. Apply this script to the mesh that you want to use to view the Mjpeg stream. 
/// </summary>
public class MjpegTexture : MonoBehaviour
{
    ...
    // Call when the value of the streamAddress property changes
    public void Restart()
    {
        Debug.Log("boo");
        mjpeg.StopStream();
        mjpeg = null; // Deallocate mjpeg

        // Check if the streamAddress property has changed
        if (Application.isPlaying && !string.IsNullOrEmpty(streamAddress))
        {
            // Call the Start method to parse the new stream address
            updateFrame = false;
            deltaTime = 0.0f;
            mjpegDeltaTime = 0.0f;
